...
public class loaiBUS {
    ...
    public List<loaiDTO> timKiem(String tieuChi, String keyword) {
        ...
        for (loaiDTO loai : dsLoai) {
            ...
            switch (tieuChi) {
                case "Mã Loại":
                    if (loai.getMaLoai().toLowerCase().contains(key)) match = true;
                    break;
                case "Tên Danh Mục":
                    if (loai.getDanhMuc().toLowerCase().contains(key)) match = true;
                    break;
                default:
                    if (loai.getMaLoai().toLowerCase().contains(key) ||
                        loai.getDanhMuc().toLowerCase().contains(key)) {
                        match = true;
                    }
                    break;
            }
            if (match) result.add(loai);
        }
        return result;
    }

    public boolean them(loaiDTO loai) {
        if (loai.getMaLoai().trim().isEmpty() || loai.getDanhMuc().trim().isEmpty()) {
            return false;
        }
