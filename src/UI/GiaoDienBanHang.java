...
public class GiaoDienBanHang extends JFrame {
    ...
    public GiaoDienBanHang() {
        setTitle("Phần Mềm Bán Hàng - POS Offline");
        setSize(1250, 780); 
    ...
    private JPanel createHeaderPanel() {
        ...
        pnlInfo.add(pnlNV); pnlInfo.add(lblNgay); pnlInfo.add(lblMaHD);
        pnlHeader.add(lblTitle, BorderLayout.CENTER); pnlHeader.add(pnlInfo, BorderLayout.SOUTH);
        return pnlHeader;
    }
    ...
    private JPanel createGioHangPanel() {
        ...
        pnl.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.GRAY, 1), "1. GIỎ HÀNG THANH TOÁN", TitledBorder.LEFT, TitledBorder.TOP, titleFont, primaryColor));
    ...
    private JPanel createChucNangPanel() {
        ...
        pnl.setBackground(Color.WHITE);
        pnl.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.GRAY, 1), "4. CHỨC NĂNG BÁN HÀNG", TitledBorder.LEFT, TitledBorder.TOP, titleFont, primaryColor));
    ...
    private void styleTable(JTable table) {
        ...
        DefaultTableCellRenderer headerRenderer = new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
                ...
                setBackground(headerColor); setForeground(Color.WHITE);
            ...
            for (SanPhamDTO sp : listSP) {
                ...
                if (listCTSP != null) {
                    for (ChitietSPDTO ct : listCTSP) {
                        ...
                        if (tt != null && (tt.toLowerCase().contains("sẵn") || tt.toLowerCase().contains("mới") || tt.trim().equals("1"))) {
                            soLuongMoi++;
                        }
                    }
                }
                modelSanPham.addRow(new Object[]{sp.getMaSP(), sp.getTenSP(), soLuongMoi, sp.getCauHinh(), dcf.format(sp.getGia())});
            }
        } catch (Exception e) { 
            e.printStackTrace(); 
        }
    ...
    private void setupEventTriggers() {
        ...
        btnTimKhach.addActionListener(e -> {
            ...
            try (java.sql.Connection conn = DAL.DAO.DBConnection.getConnection(); 
                 java.sql.PreparedStatement ps = conn.prepareStatement("SELECT HOTEN FROM khachhang WHERE SDT = ?")) {
                ps.setString(1, sdt);
        ...
        btnThemKhach.addActionListener(e -> {
            String sdt = txtSdtKhach.getText().trim();
            if (!sdt.matches("^0\\d{9}$")) {
                JOptionPane.showMessageDialog(this, "Nhập SĐT hợp lệ trước khi thêm!"); return;
            }
            String ten = JOptionPane.showInputDialog(this, "Nhập Họ Tên khách mới:");
            if (ten == null || ten.trim().isEmpty()) return;
            
            try (java.sql.Connection conn = DAL.DAO.DBConnection.getConnection(); 
                 java.sql.PreparedStatement ps = conn.prepareStatement("INSERT INTO khachhang (MAKH, HOTEN, SDT) VALUES (?, ?, ?)")) {
        ...
        btnThemSP.addActionListener(e -> {
            try {
                ...
                if (realIMEI != null) {
                    modelGioHang.addRow(new Object[]{modelGioHang.getRowCount() + 1, realIMEI, tenSP, 12, dcf.format(giaBan), dcf.format(giaBan)});
                    capNhatTongTien();
                } else {
                    JOptionPane.showMessageDialog(this, "Máy này đã bốc hết vô giỏ rồi (hoặc lỗi DB không lấy được IMEI)!");
                }
        ...
        btnXuatHoaDon.addActionListener(e -> {
            ...
            try {
                ...
                for (int i = 0; i < modelGioHang.getRowCount(); i++) {
                    ...
                    if(ten.length() > 20) ten = ten.substring(0, 17) + "..."; // Cắt bớt tên nếu quá dài
        ...
        btnXacNhan.addActionListener(e -> {
            ...
            try {
                ...
                try (java.sql.Connection conn = DAL.DAO.DBConnection.getConnection()) {
                    String maKH = "";
                    // Tìm Mã KH
                    try (java.sql.PreparedStatement psKH = conn.prepareStatement("SELECT MAKH FROM khachhang WHERE SDT = ?")) {
                        psKH.setString(1, sdt);
                        java.sql.ResultSet rsKH = psKH.executeQuery();
                        if (rsKH.next()) maKH = rsKH.getString("MAKH");
                    }
                    if (maKH.isEmpty()) {
        ...
        btnXemLichSu.addActionListener(e -> {
            JDialog dialog = new JDialog(this, "Lịch Sử Giao Dịch", true);
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new GiaoDienBanHang().setVisible(true));
    }
}