...
public class SanPhamPanel extends JPanel {
    ...
    private JPanel taoPhanDau() {
        ...
        pnlFields.setBorder(new CompoundBorder(
                ...
                new CompoundBorder(
                        new TitledBorder(new EmptyBorder(0, 0, 0, 0), " Thông số chi tiết ", TitledBorder.LEFT,
                                TitledBorder.TOP, new Font("Segoe UI", Font.BOLD, 12), ACCENT_BLUE),
        ...
        // Row 1
        pnlFields.add(taoNhan("Mã SP:"));
        pnlFields.add(txtMaSP);
        pnlFields.add(taoNhan("Tên SP:"));
        pnlFields.add(txtTenSP);
        // Row 2
        pnlFields.add(taoNhan("Danh mục:"));
        pnlFields.add(cbMaLoai);
        pnlFields.add(taoNhan("Nhà SX:"));
        pnlFields.add(cbNSX);
        // Row 3
        pnlFields.add(taoNhan("Trạng thái:"));
        pnlFields.add(cbTrangThai);
        pnlFields.add(taoNhan("SL tồn:"));
        pnlFields.add(txtSLTon);
        // Row 4
        pnlFields.add(taoNhan("Cấu hình:"));
        pnlFields.add(txtCauHinh);
        pnlFields.add(taoNhan("Giá nhập (VNĐ):"));
        pnlFields.add(txtGiaNhap);
        // Row 5
        pnlFields.add(taoNhan("% Lợi nhuận:"));
        pnlFields.add(txtLoiNhuan);
        pnlFields.add(taoNhan("Giá bán (VNĐ):"));
        pnlFields.add(txtGiaBan);
    ...
    private JPanel taoPhanDuoi() {
        ...
        pnlBtns.add(btnLuuDB);

        pnlToolbar.add(pnlSearch, BorderLayout.WEST);
        pnlToolbar.add(pnlBtns, BorderLayout.EAST);

        model = new DefaultTableModel(new String[] { "Hình ảnh", "Mã SP", "Tên SP", "Cấu hình", "Tồn", "Giá nhập",
                "% Lời", "Giá bán", "Trạng thái", "Loại", "Hãng" }, 0) {
            @Override
        ...
        btnXoa.addActionListener(e -> {
            ...
            if (JOptionPane.showConfirmDialog(this,
                    "Xóa Sản Phẩm này sẽ xóa luôn các Chi Tiết SP thuộc về nó. Tiếp tục?", "Cảnh báo",
        ...
        btnLuuDB.addActionListener(e -> {
            if (spBUS.saveToDatabase())
                JOptionPane.showMessageDialog(this, "Đã cập nhật dữ liệu xuống Database!");
        });
    ...
    private void xuLyHanhDong(String mode) {
        ...
        if (!maSP.matches("^SP\\d+$")) {
            JOptionPane.showMessageDialog(this,
                    "Mã sản phẩm không hợp lệ!\nVui lòng nhập đúng định dạng SP + Số (Ví dụ: SP01, SP123,...)",
        ...
        for (SanPhamDTO s : list) {
            ...
            for (anhspDTO a : imgBUS.getAll()) {
                if (a.getMaSP().equals(s.getMaSP())) {
                    try {
                        icon = new ImageIcon(
                                new ImageIcon(a.getUrl()).getImage().getScaledInstance(30, 30, Image.SCALE_SMOOTH));
                    } catch (Exception e) {
                        icon = null;
                    }
                    break;
            ...
            for (Map.Entry<String, String> entry : mapLoai.entrySet()) {
                if (entry.getValue().equals(s.getMaLoai()))
                    tenL = entry.getKey();
            }
            for (Map.Entry<String, String> entry : mapNSX.entrySet()) {
                if (entry.getValue().equals(s.getNsx()))
                    tenH = entry.getKey();
            }
    ...
    private void thietLapSuKienBang() {
        table.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                for (anhspDTO a : imgBUS.getAll()) {
                    if (a.getMaSP().equals(msp)) {
                        capNhatXemTruoc(a.getUrl());
                        return;
                    }
                }
                lblHinhAnh.setIcon(null);
                lblHinhAnh.setText("Chưa có ảnh của SP này");
            }
        });
    }
    ...
    private void capNhatXemTruoc(String path) {
        try {
            ImageIcon icon = new ImageIcon(
                    new ImageIcon(path).getImage().getScaledInstance(260, 220, Image.SCALE_SMOOTH));
    ...
    private void lamMoiCombobox() {
        ...
        mapLoai.clear();
        for (loaiDTO l : lBUS.getAll()) {
            cbMaLoai.addItem(l.getDanhMuc());
            mapLoai.put(l.getDanhMuc(), l.getMaLoai());
        }
        cbNSX.removeAllItems();
        mapNSX.clear();
        for (hangsxDTO h : hBUS.getAll()) {
            cbNSX.addItem(h.getTenTH());
            mapNSX.put(h.getTenTH(), h.getMaNSX());
        }
    }
    ...
    private void lamSachForm() {
        ...
        txtGiaBan.setText("0 đ");
        lblHinhAnh.setIcon(null);
        lblHinhAnh.setText("Chọn SP để xem ảnh");
    }
